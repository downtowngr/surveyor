  ...
  describe "#process_text" do
    ...
    describe "when a citizen votes twice for the same choice" do
      it "should not increment the vote" do
        expect { MultipleChoiceStrategy.process_text(poll_choice1, citizen.current_votes(poll), text1, citizen) }.to change { poll_choice1.votes.count }.by(1)
        expect { MultipleChoiceStrategy.process_text(poll_choice1, citizen.current_votes(poll), text1, citizen) }.not_to change { poll_choice1.votes.count }
      end
    end
    ...
    describe "when a citizen votes for different choices" do
      it "should increment the votes for both" do
        expect { MultipleChoiceStrategy.process_text(poll_choice1, citizen.current_votes(poll), text1, citizen) }.to change { poll_choice1.votes.count }.by(1)
        expect { MultipleChoiceStrategy.process_text(poll_choice2, citizen.current_votes(poll), text2, citizen) }.to change { poll_choice2.votes.count }.by(1)
      end
    end
  end
end
